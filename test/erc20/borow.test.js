...
describe('AAVE V2 Data ', function () {
  ...
  it('web3 borrow repay', async () => {
    this.timeout(50000);
    let erc20Token = this.DAI;

    let reserveAddr = '0x2d5Ed9bC72E93696D985ddB3684e5BAD6F9e1D11';
    let amount = (1e18).toString();
    ...
    let res1 = await this.lpContractProxy.web3Obj.methods
      .borrow(reserveAddr, amount, 2, 0, account)
      .send({
        from: account,
      });
    ...
    let res2 = await this.lpContractProxy.repay(erc20Token.address, amount, 2, account, {
      from: account,
    });
