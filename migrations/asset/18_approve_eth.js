...
let depoistToken = async (ethGateWay, account, mintTotal) => {
  ...
  let bal = new BN(depositAmount).div(ethDecimalBN).toString();
  console.log('depoistToken', bal);
...
let withdrawToken = async (ethGateWay, account, mintTotal) => {
  let withdrawAmount = new BN(4000);
...
let borrowStable = async (ethGateWay, account, mintTotal) => {
  let borrowAmount = mintTotal.div(new BN(4000));
  await ethGateWay.borrowETH(borrowAmount, 1, 0, { from: account });
};
...
module.exports = async (deployer, network, accounts) => {
  ...
  let wEthReserve = await lpContract.getReserveData(weth.address);

  let aWETHAddr = await wETHGateway.getAWETHAddress();
  if (aWETHAddr != wEthReserve.aTokenAddress) {
    console.error('wETHGateway.getAWETHAddress error');
    return;
  }
  ...
  let reserves = await apiDataProvider.getReserves();

  for (let addr of reserves) {
    let reserveData = await apiDataProvider.getReserveData(addr);
